...
def generate_description_question(
    ...
    """Generate descriptive questions for time series, focusing on periodicity, trend, level, and spikes"""

    # Generate visualization
    if TYPE == "image":
        img_path = plot_time_series(ts, features)
        question_intro = (
            f"<image>Please describe the main characteristics of this time series.\n\n"
        )
    elif TYPE == "text":
        img_path = []
        question_intro = f"Input Data:\nTime Series = {ts.tolist()}\n\nPlease describe the main characteristics of this time series.\n\n"
    elif TYPE == "text_and_image":
        img_path = plot_time_series(ts, features)
    ...
    problem_description = (
        f"{question_intro}\n"
        "Requirements:\n"
        "1. Describe the periodic characteristics (e.g., number of periods, period lengths, and patterns within periods)\n"
        "2. Describe the trend characteristics (e.g., growth, decline, or stable trends)\n"
        "3. Describe any anomalous spikes (e.g., sudden increases or decreases)\n"
        "4. Describe the overall level changes in the time series\n\n"
    )

    # Generate answer description
    # 1. Periodicity description
    periodicity_desc = "1. Periodic Characteristics:\n"
    if features.periods:
        main_period = features.main_period
        num_complete_periods = features.length // main_period  # 完整周期数
        
        # 分析一个完整周期内的变化趋势
        one_period = ts[:main_period]
        overall_amplitude = np.max(one_period) - np.min(one_period)  # 整体振幅
        
        # 对一个周期内的数据进行滑动窗口平滑处理
        window_size = max(3, main_period // 10)  # 窗口大小设为周期长度的1/10，最小为3
        if window_size % 2 == 0:  # 确保窗口大小为奇数
            window_size += 1
        window = np.ones(window_size) / window_size
        # 使用卷积进行平滑处理，mode='same'保持序列长度不变
        smoothed_period = np.convolve(one_period, window, mode='same')
        
        # 分析周期内的变化模式
        pattern = analyze_period_pattern(smoothed_period)
        
        # 在平滑后的数据上找出关键点
        peaks, peak_props = find_peaks(smoothed_period, prominence=0.1)  # 使用prominence参数过滤出显著的峰值
        valleys, valley_props = find_peaks(-smoothed_period, prominence=0.1)  # 谷值分析
        
        periodicity_desc += f"   - Dominant Period Length: {main_period} time units\n"
        periodicity_desc += f"   - Number of Complete Cycles: {num_complete_periods}\n"
        periodicity_desc += f"   - Overall Amplitude Range in One Period: {overall_amplitude:.3f}\n"
        periodicity_desc += f"   - Within-Period Pattern: {pattern}\n"
        
        if len(peaks) > 0 or len(valleys) > 0:
            periodicity_desc += "   - Key Points in One Period:\n"
            if len(peaks) > 0:
                # 添加峰值的prominence信息
                peak_info = [f"{p}/{main_period}(prominence={peak_props['prominences'][i]:.3f})" 
                           for i, p in enumerate(peaks)]
                periodicity_desc += f"     * Peak(s) at relative position in the first period: {', '.join(peak_info)}\n"
            if len(valleys) > 0:
                # 添加谷值的prominence信息
                valley_info = [f"{v}/{main_period}(prominence={valley_props['prominences'][i]:.3f})" 
                             for i, v in enumerate(valleys)]
                periodicity_desc += f"     * Valley(s) at relative position in the first period: {', '.join(valley_info)}\n"
        
        # 添加频域异常的描述
        if features.freq_changes:
            periodicity_desc += "   - Frequency Anomalies Detected:\n"
            for change in features.freq_changes:
                ...
                if change["subtype"] == "frequency":
                    ...
                    if freq_multiplier > 1:
                        periodicity_desc += f"     * Frequency increase (x{freq_multiplier:.1f}) from position {pos} to {pos+length}\n"
                    else:
                        periodicity_desc += f"     * Frequency decrease (x{freq_multiplier:.1f}) from position {pos} to {pos+length}\n"
                else:  # amplitude
                    amplitude_factor = change["amplitude_factor"]
                    if amplitude_factor > 1:
                        periodicity_desc += f"     * Amplitude increase (x{amplitude_factor:.1f}) from position {pos} to {pos+length}\n"
                    else:
                        periodicity_desc += f"     * Amplitude decrease (x{amplitude_factor:.1f}) from position {pos} to {pos+length}\n"
        
    else:
        periodicity_desc += "   - No significant periodic patterns detected in the time series\n"

    # 2. Trend description
    trend_desc = "2. Trend Characteristics:\n"
    if features.trend_direction == "increase":
        trend_desc += f"   - Overall shows an increasing trend with slope approximately {features.trend_slope:.3f}\n"
    elif features.trend_direction == "decrease":
        trend_desc += f"   - Overall shows a decreasing trend with slope approximately {features.trend_slope:.3f}\n"
    else:
        trend_desc += "   - Overall trend is relatively stable\n"
    trend_desc += f"   - Trend strength is {features.trend_strength:.3f}\n"
    ...
    # 3. Spike description
    spike_desc = "3. Spike Characteristics:\n"
    if features.spikes:
        spike_desc += "   - The following anomalous spikes were detected:\n"
        for spike in features.spikes:
            pos = spike["position"]
            mag = spike["magnitude"]
            if mag > 0:
                spike_desc += (
                    f"   - Sudden increase at position {pos} with magnitude {mag:.3f}\n"
                )
            else:
                spike_desc += (
                    f"   - Sudden decrease at position {pos} with magnitude {mag:.3f}\n"
                )
    else:
        spike_desc += "   - No significant spikes detected\n"

    # 4. Level description
    level_desc = "4. Level Characteristics:\n"
    if features.level_shifts:
        level_desc += "   - The following level shifts were detected:\n"
        for shift in features.level_shifts:
            pos = shift["position"]
            mag = shift["magnitude"]
            if mag > 0:
                level_desc += (
                    f"   - Level increase at position {pos} with magnitude {mag:.3f}\n"
                )
            else:
                level_desc += f"   - Level decrease at position {pos} with magnitude {abs(mag):.3f}\n"
    else:
        mean_val = features.mean
        std_val = features.std
        level_desc += f"   - Overall level is stable with mean {mean_val:.3f} and standard deviation {std_val:.3f}\n"
